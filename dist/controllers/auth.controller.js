...
let authservice = new auth_services_1.AuthService();
...
const toSafeUser = (user) => {
    const userObject = user && typeof user.toObject === "function" ? user.toObject() : user || {};
...
class AuthController {
    async registerUser(req, res) {
        try {
            const parsedData = auth_dto_1.createUserDto.safeParse(req.body);
    ...
    async loginUser(req, res) {
        try {
            const parsedData = auth_dto_1.LoginUserDto.safeParse(req.body);
            if (!parsedData.success) {
                return res.status(400).json({
                    success: false,
                    message: "Validation failed",
    ...
    async updateUser(req, res) {
        try {
            ...
            const { status: _status, role: _role, _id: _id, document: _document, ...updateData } = req.body || {};
    ...
    async uploadUserDocument(req, res) {
        ...
        try {
            ...
            const updatedUser = await authservice.saveUserDocument(user._id.toString(), user.role, file);
            return res.status(200).json({
                success: true,
                data: toSafeUser(updatedUser),
    ...
    async downloadMyDocument(req, res) {
        try {
            const user = req.user;
            const document = await authservice.getUserDocument(user._id.toString());
            return res.download(document.absolutePath, document.metadata.originalName);
        }
        catch (error) {
            return res.status(error.statusCode || 500).json({
                success: false,
    ...
    async deleteMyDocument(req, res) {
        try {
            const user = req.user;
            const updatedUser = await authservice.deleteUserDocument(user._id.toString());
            return res.status(200).json({
                success: true,
                data: toSafeUser(updatedUser),
    ...
    async updateUserStatus(req, res) {
        try {
            ...
            const parsedData = auth_dto_1.UpdateUserStatusDto.safeParse(req.body);
            if (!parsedData.success) {
                return res.status(400).json({
                    success: false,
                    message: "Validation failed",
            ...
            const updatedUser = await authservice.updateUserStatus(userId, parsedData.data.status);
            return res.status(200).json({
                success: true,
                data: toSafeUser(updatedUser),
